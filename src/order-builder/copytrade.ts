...
function loadState(): SimpleStateFile {
    ...
    try {
        ...
        console.log(`Failed to read state: ${e instanceof Error ? e.message : String(e)}`);
    }
    return {};
}
...
function saveState(state: SimpleStateFile): void {
    ...
    saveStateTimer = setTimeout(() => {
        try {
            ...
            console.log(`Failed to save state: ${e instanceof Error ? e.message : String(e)}`);
        }
        saveStateTimer = null;
...
export class CopytradeArbBot {
    ...
    async initializeWebSocket(): Promise<void> {
        ...
        try {
            ...
            console.log("WebSocket orderbook initialized");
        } catch (e) {
            console.log(`Failed to initialize WebSocket: ${e instanceof Error ? e.message : String(e)}`);
            throw e;
        }
    }

    async start(): Promise<void> {
        logger.info("Starting CopytradeArbBot");
        if (this.isStopped) {
            console.log("Bot is stopped, cannot start");
            return;
        }

        if (!this.wsOrderBook) {
            console.log("Fatal error: WebSocket orderbook not initialized - cannot start bot");
            return;
        }

        console.log(`Starting CopytradeArbBot for markets: ${this.cfg.markets.join(", ")}`);
    ...
    stop(): void {
        ...
        console.log("CopytradeArbBot stopped");
    }
    ...
    private async initializeMarket(market: string): Promise<void> {
        try {
            ...
            console.log(`Initializing market ${market} with slug ${slug}`);
            ...
            const slug = slugForCurrent15m(market);
            console.log(`⚠️  Market ${market} not available yet (${slug}): ${errorMsg}. Will retry on next price update.`);
    ...
    private async handlePriceUpdate(
        ...
        queueMicrotask(async () => {
            ...
            if (prevSlug && prevSlug !== slug) {
                console.log(`🔄 New market cycle detected for ${market}: ${prevSlug} → ${slug}`);
                ...
                console.log(`🔄 Re-initializing market ${market} with new slug ${slug}`);
                try {
                    ...
                    console.log(`✅ Market ${market} re-initialized with new token IDs`);
                } catch (e) {
                    const errorMsg = e instanceof Error ? e.message : String(e);
                    console.log(`⚠️  Failed to re-initialize market ${market} with new slug ${slug}: ${errorMsg}. Will retry on next price update.`);
            ...
            if (lastPred) {
                ...
                console.log(`🔮 Prediction: ${lastPred.prediction.direction.toUpperCase()} (conf: ${lastPred.prediction.confidence.toFixed(2)}) | Actual: ${actualDirection.toUpperCase()} | ${wasCorrect ? "✅ CORRECT" : "❌ WRONG"} | Time: ${timeDiff}ms`);
            ...
            if (stats.totalPredictions > 0) {
                ...
                if (stats.totalPredictions % 25 === 0) {
                    console.log(`📊 Prediction Accuracy: ${(stats.accuracy * 100).toFixed(1)}% (${stats.correctPredictions}/${stats.totalPredictions})`);
                }
    ...
    private async checkAndHandleMarketCycleChanges(): Promise<void> {
        ...
        for (const market of this.cfg.markets) {
            ...
            if (prevSlug && prevSlug !== currentSlug) {
                console.log(`🔄 Market cycle change detected via periodic check for ${market}: ${prevSlug} → ${currentSlug}`);
    ...
    private async reinitializeMarketForNewCycle(market: string, prevSlug: string, newSlug: string): Promise<void> {
        console.log(`🔄 Re-initializing market ${market} with new slug ${newSlug} (from periodic check)`);
        ...
        try {
            ...
            console.log(`✅ Market ${market} re-initialized with new token IDs for cycle ${newSlug}`);
        } catch (e) {
            const errorMsg = e instanceof Error ? e.message : String(e);
            console.log(`⚠️  Failed to re-initialize market ${market} with new slug ${newSlug}: ${errorMsg}. Will retry on next check.`);
        }
    }
    ...
    private async buyShares(
        ...
        // Log buy
        console.log(`BUY: ${leg} ~${estimatedShares} shares @ limit ${limitPrice.toFixed(4)} (${orderAmount.toFixed(2)} USDC)`);
        ...
        try {
            ...
            if (!orderID) {
                console.log(`BUY failed for ${leg} - no orderID returned`);
                return false;
            }
            // Order placed successfully
            console.log(`✅ First-Side Order placed: ${leg} orderID ${orderID.substring(0, 10)}... @ ${limitPrice.toFixed(4)}`);
            return true;
        } catch (e) {
            console.log(`BUY failed for ${leg}: ${e instanceof Error ? e.message : String(e)}`);
            return false;
        }
    }
    ...
    private executePredictionTrade(
        ...
        if (hasPerSideLimit) {
            if (buyToken === "UP" && tokenCounts.upTokenCount >= this.MAX_BUY_COUNTS_PER_SIDE) {
                console.log(`⛔ LIMIT REACHED: UP count is ${tokenCounts.upTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE} - skipping trade`);
                return; // Skip - limit reached
            }
            if (buyToken === "DOWN" && tokenCounts.downTokenCount >= this.MAX_BUY_COUNTS_PER_SIDE) {
                console.log(`⛔ LIMIT REACHED: DOWN count is ${tokenCounts.downTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE} - skipping trade`);
                return; // Skip - limit reached
            }
        }
        ...
        console.log(`🎯 FIRST-SIDE Trade: ${buyToken} @ ${buyPrice.toFixed(4)} (${buyCost.toFixed(2)} USDC) | UP ${tokenCounts.upTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE}, DOWN ${tokenCounts.downTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE} | Limit: ${this.MAX_BUY_COUNTS_PER_SIDE} per side`);
        ...
        if (hasPerSideLimit &&
            ...
            console.log(`⏸️  Market ${scoreKey} PAUSED: Reached limit (UP: ${tokenCounts.upTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE}, DOWN: ${tokenCounts.downTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE})`);
        }
    }
    ...
    private async placeSecondSideLimitOrder(
        ...
        if (limitPrice <= 0 || limitPrice >= 1) {
            console.log(`⚠️  Invalid limit price calculated: ${limitPrice.toFixed(4)} (from first side price ${firstSidePrice.toFixed(4)})`);
            return;
        }
        ...
        try {
            ...
            if (orderID) {
                ...
                console.log(`⚠️  Second-side limit order placement returned no orderID`);
            }
        } catch (e) {
            console.log(`❌ Failed to place limit order for ${oppositeSide} token: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
    ...
    private async trackLimitOrderAsync(
        ...
        try {
            ...
            while (attempts < maxAttempts) {
                ...
                try {
                    ...
                    if (order && order.status === "FILLED") {
                        ...
                        if (wouldExceedLimit) {
                            console.log(`⚠️  Limit order ${orderID} filled but would exceed limit - cancelling count update (${leg}: ${leg === "YES" ? tokenCounts.upTokenCount : tokenCounts.downTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE})`);
                        ...
                        console.log(`✅ Limit order filled: ${leg} @ ${limitPrice.toFixed(4)} | UP ${tokenCounts.upTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE}, DOWN ${tokenCounts.downTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE}`);
                        ...
                        if (hasPerSideLimit &&
                            ...
                            tokenCounts.downTokenCount >= this.MAX_BUY_COUNTS_PER_SIDE) {
                            this.pausedMarkets.add(scoreKey);
                            console.log(`⏸️  Market ${scoreKey} PAUSED after limit order fill: UP: ${tokenCounts.upTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE}, DOWN: ${tokenCounts.downTokenCount}/${this.MAX_BUY_COUNTS_PER_SIDE}`);
                        }
            ...
            console.log(`❌ Error tracking limit order ${orderID}: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
    ...
    private generatePredictionScoreSummary(prevSlug: string, market: string): void {
        ...
        const score = this.predictionScores.get(scoreKey);
        if (!score) {
            console.log(`⚠️  No prediction score found for ${scoreKey} - cannot generate summary`);
            return;
        }
        ...
        console.log(`\n${"=".repeat(80)}`);
        console.log(`📊 PREDICTION SCORE SUMMARY - Market: ${market} | Slug: ${prevSlug}`);
        console.log(`${"=".repeat(80)}`);
        console.log(`⏱️  Duration: ${(duration / 60).toFixed(2)} minutes`);
        console.log(`📈 Total Predictions: ${score.totalPredictions}`);
        console.log(`✅ Correct Predictions: ${score.correctPredictions}`);
        console.log(`❌ Wrong Predictions: ${score.totalPredictions - score.correctPredictions}`);
        console.log(`🎯 Success Rate: ${successRate.toFixed(2)}%`);
        console.log(`\n💰 TOKEN PURCHASES:`);
        console.log(`   UP Token:`);
        console.log(`      - Buy Count: ${score.upTokenCount}`);
        console.log(`      - Total Cost: ${score.upTokenCost.toFixed(2)} USDC`);
        console.log(`   DOWN Token:`);
        console.log(`      - Buy Count: ${score.downTokenCount}`);
        console.log(`      - Total Cost: ${score.downTokenCost.toFixed(2)} USDC`);
    ...
    private async trackOrderAsync(
            ...
            try {
                ...
                if (status === "MATCHED" || status === "FILLED" || status === "PARTIALLY_FILLED" || sizeMatched > 0) {
                    ...
                    if (newlyFilled > 0) {
                        ...
                        console.log(`✅ ${leg} order ${orderID} filled: ${newlyFilled} shares (total matched: ${sizeMatched}/${estimatedShares})`);
                        lastLoggedSize = sizeMatched;
                    }

                    // If fully filled, we're done
                    if (status === "FILLED" || status === "MATCHED" || sizeMatched >= estimatedShares) {
                        console.log(`✅ ${leg} order ${orderID} fully filled: ${sizeMatched} shares`);
                        return;
                    }
                }
                ...
                if (status === "CANCELLED" || status === "FAILED") {
                    console.log(`Order ${orderID} ${status.toLowerCase()}`);
                    return;
                }
                ...
                console.log(`Error tracking order ${orderID}: ${e instanceof Error ? e.message : String(e)}`);
                if (attempt < maxAttempts - 1) {
                    await new Promise(resolve => setTimeout(resolve, delayMs));
                }
            }
        ...
        console.log(`Order ${orderID} tracking completed after ${maxAttempts} attempts (may still be pending)`);
    }
}
